...
2. **完成后立即提交（Agent 自动执行，无需询问用户）**：完成独立功能、修复 bug、重构模块后立即提交。施工文档更新与代码改动合并为**一个 commit**（一个任务 = 一个逻辑变更），commit message 带 T 编号。详见 `task-workflow` skill 其三第 7 条。
...
以下命令在 PowerShell trae-sandbox 环境中会卡死（触发分页器），**禁止使用**：

- `git diff --cached --name-only` → 改用 `git status`
- `git diff --stat` → 改用 `git status`
- 任何可能触发 less/more 分页器的 git 命令 → 加 `--no-pager` 前缀或改用 `git status`
