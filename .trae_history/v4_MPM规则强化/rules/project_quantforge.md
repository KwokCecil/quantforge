...
## 规则同步规范
- `.trae/rules/` 中的规则文件是 VS Code 实际加载的生效版本（位于 Git 仓库外）
- `.trae_history/` 是规则的版本历史存档（位于 Git 仓库内，`quantforge/.trae_history/`）
- **规则有小改动时**：同步到 `.trae_history/v3_精简版_当前/` 对应文件
- **规则有大改动时**（如架构重塑、新增章节超30行）：新建 `v{序号}_{描述}/` 目录，复制全部规则和技能文件进去，再覆盖更新当前版本
- 规则同步必须随相关代码变更一起 git commit，保持规则版本可追溯